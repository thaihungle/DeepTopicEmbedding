...
def count_data(filename):
  ...
  for l in fp:
    numdoc+=1
    for w in l.split()[1:]:
      v=int(float(w.split(':')[0]))
      if num_terms<v:
	num_terms=v
      if v<off:
	off=v
  return numdoc,num_terms-off+1

def convert_multilabel_unsupervised(mulfile, unsupfile):
  fp = open(mulfile, 'r')
  fw = open(unsupfile,'w')
  for l in fp:
    writestr=''
    ...
    for i in range(len(sen)) :
      if i>0:
	wlist.append(int(float(sen[i].split(':')[0]))-1)
	countlist.append(sen[i].split(':')[1])
	writestr+=str(wlist[i-1])+':'+countlist[i-1]+' '
    numw=len(set(wlist))
    writestr=str(numw)+' '+writestr
...
def read_data(filename):
    ...
    while True:
        ...
        for j in range(1, doc_length + 1):
            ...
            ids[j - 1] = int(float(term_count[0]))
            cts[j - 1] = int(float(term_count[1]))
        wordids.append(ids)
        wordcts.append(cts)
    fp.close()
...
def read_minibatch_dict(fp, batch_size):
    ...
    for i in range(batch_size):
        ...
        cts = list()
        terms = string.split(line)
        for j in range(1,int(float(terms[0])) + 1):
            term_count = terms[j].split(':')
            ids.append(int(float(term_count[0])))
            cts.append(int(float(term_count[1])))
