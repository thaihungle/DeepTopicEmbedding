...
void free_lda_suffstats(lda_suffstats *ss, int num_topics)
{
	int i;
	for (i = num_topics -1; i >= 0; i--)	free(ss->class_word[i]);
	free(ss->class_total);
	free(ss);
}
...
{
    int i;
    for (i = model->num_topics -1; i >= 0; i--)	free(model->log_prob_w[i]);
    free(model);
}
...
void save_lda_model(lda_model* model, char* model_root)
{
    ...
    for (i = 0; i < model->num_topics; i++)
    {
		for (j = 0; j < model->num_terms; j++)
			fprintf(fileptr, "%1.10f ", exp(model->log_prob_w[i][j]));
		fprintf(fileptr, "\n");
    }
    fclose(fileptr);
...
lda_model* load_lda_model(char* model_root)
{
    ...
    for (i = 0; i < num_topics; i++)
    {
        ...
        {
            fscanf(fileptr, "%f", &x);
            model->log_prob_w[i][j] = x + (1e-10);	//loss information from saving
        }
		L1_normalize(model->log_prob_w[i], num_terms);
		for (j = 0; j < num_terms; j++) model->log_prob_w[i][j] = log(model->log_prob_w[i][j]);
    }
    fclose(fileptr);
    return(model);
}
